...
describe('[Challenge] The rewarder', function () {
    ...
    it('Execution', async function () {
        ...
        await ethers.provider.send("evm_increaseTime", [5 * 24 * 60 * 60]); // 5 days
        this.attackerContract = await (await ethers.getContractFactory("AttackRewarder", player)).deploy(
...
describe('[Test] The rewarder', function () {
    const TOKENS_IN_LENDER_POOL = 1000000n * 10n ** 18n; // 1 million tokens
    let users, deployer, alice, bob, charlie, david, player;
    let liquidityToken, flashLoanPool, rewarderPool, rewardToken, accountingToken;
    ...
    it('Initial Player tokens', async function () {
        expect(await liquidityToken.balanceOf(player.address)).to.eq(0);
    });
    it('Reward of every user equal to supply', async function () {
        expect(await rewardToken.totalSupply()).to.be.eq(await rewarderPool.REWARDS());
    });
    it('2 rounds occured', async function () {
        expect(await rewarderPool.roundNumber()).to.be.eq(2);
    });
});
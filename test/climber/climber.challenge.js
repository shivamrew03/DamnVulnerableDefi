...
describe('[Challenge] Climber', function () {
    ...
    it('Execution', async function () {
        ...
        AttackContract = await(await ethers.getContractFactory('AttackerClimberTimelock',player)).deploy(
            timelock.address,
            vault.address,
            token.address
        )
        await AttackContract.attack();
        // await vault.sweepFunds(token.address, player.address);
    });
...
describe('[Tests] Climber', function () {
    let deployer, proposer, sweeper;
    let timelock, vault, token;
    ...
    before(async function () {
        /** SETUP SCENARIO - NO NEED TO CHANGE ANYTHING HERE */
        [deployer, proposer, sweeper] = await ethers.getSigners();
    ...
    it('Sweeper Should be correctly assigned', async function () {
        expect(await vault.getSweeper()).to.eq(sweeper.address);
    });
    it('No withdrawal should have happened', async function () {
        expect(await vault.getLastWithdrawalTimestamp()).to.be.gt(0);
    });
    
    it('Vault owner should be correct', async function () {
        expect(await vault.owner()).to.not.eq(ethers.constants.AddressZero);
        expect(await vault.owner()).to.not.eq(deployer.address);    
    });

    it('Ensure timelock delay is correct and cannot be changed', async function () {
        expect(await timelock.delay()).to.eq(TIMELOCK_DELAY);
        await expect(timelock.updateDelay(TIMELOCK_DELAY + 1)).to.be.revertedWithCustomError(timelock, 'CallerNotTimelock');
    });

    it('Ensure timelock roles are correctly initialized', async function () {
        expect(
            await timelock.hasRole(ethers.utils.id("PROPOSER_ROLE"), proposer.address)
        ).to.be.true;
        expect(

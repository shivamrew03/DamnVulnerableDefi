...
//         ).to.be.eq(POOL_INITIAL_TOKEN_BALANCE * 2n);
//     });
//     it('should allow borrowing if sufficient collateral is provided', async () => {
//       const amount = 100;
//       const depositRequired = await lendingPool.calculateDepositRequired(amount);
  
//       await lendingPool.connect(player).borrow(amount, deployer.address, { value: depositRequired });
  
//       const borrowedAmount = await lendingPool.deposits(player.address);
//       expect(borrowedAmount).to.equal(depositRequired);
//     });
  
    
  
//     it('should refund excess collateral', async () => {
//       const amount = 100;
//       const depositRequired = await lendingPool.calculateDepositRequired(amount);
//       const excessDeposit = 10;
  
//       const borrowerBalanceBefore = await ethers.provider.getBalance(player.address);
  
//       await lendingPool.connect(player).borrow(amount, deployer.address, { value: depositRequired + excessDeposit });
  
//       const borrowerBalanceAfter = await ethers.provider.getBalance(player.address);
//       expect(borrowerBalanceBefore).to.be.greaterThan(borrowerBalanceAfter);
//     });
